...
When opening pull requests from cloud agents, summarize what was changed, why, and concrete **local** verification steps (e.g. schemes to build, smoke tests). When that PR is done (complete, not a draft), merge it with `gh pr merge` after required checks pass, or enable auto-merge. If merge is blocked, report the blocker; do not force-push or admin-bypass. Merge only that PR.
